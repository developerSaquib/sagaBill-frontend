...
import { formSchema } from "@/components/Service-Sessions/schema/formSchema";
...
const useAddEditServiceSessions = () => {
  ...
  const emptyTabObj: SaleTabInterface = {
    ...
    modifiedDate: "",
    costPrice: 0, // Default string (should be populated on update)
  };
  useEffect(() => {
  ...
  const handleSubmit = async (data?: any) => {
    ...
    const payload = PayloadModify(formSchema, { ...data, isService: 1 });
    try {
